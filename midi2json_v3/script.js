...
function getStr(uarr, start, length) {
  const end = start + length;
  const arr = uarr.slice(start, end);
  let str = '';
  for (const i of arr) str += String.fromCharCode(i);
  return str;
}
function getVLQ(uarr, start) {
  let num = 0;
  let length = 0;
  let buffer;
  let ptr = start;
...
function getUint(uarr, start, length) {
  const arr = uarr.slice(start, start + length);
  let num = 0;
...
function analyse() {
  ...
  reader.onload = function() {
    ...
    let p = 0; // 类似于读取文件的指针
    if (getStr(midi, p, 4) === 'MThd') { // 暂未添加文件长度小于4错误信息
      p += 4;
      const thdlen = getUint(midi, p, 4);
      p += 4;
      const buffer = midi.slice(p, p + thdlen);
      data.type = getUint(buffer, 0, 2);
      data.trkNum = getUint(buffer, 2, 2);
      data.time = getUint(buffer, 4, 2); // 暂未考虑SMPTE
      ...
      for (let i = 0; i < data.trkNum; i++) {
        if (getStr(midi, p, 4) === 'MTrk') { // 暂未添加文件长度小于4错误信息
          p += 4;
          ...
          while (p < trklen) {
            ...
            switch (Math.floor(type / 16)) {
              ...
              case 15: // 0xF
                if (type === 255) { // 0xFF
                  const id = midi[p++];
                  const evtVLQ = getVLQ(midi, p);
                  const evtLen = evtVLQ.num;
                  p += evtVLQ.length;
                  if (id === 81) { // 0x51
                    meta.tick = tick;
                    meta.tempo = getUint(midi, p, evtLen);
                    tempo.push(meta);
                  }
                  p += evtLen;
                }
    ...
    for (const i of Object.keys(trkList)) createOption(i);
    if (drum.length) createOption(-1);
    const end = Date.now();
    out.className = 'accept';
    out.innerHTML = `分析成功(${(end - start) / 1000}s)：可转换${trkList.length + Boolean(drum.length)}条音轨<br>`;
...
function convert() {
  ...
  for (const i of Object.keys(trkCur)) {
    ...
    if (trkCur[i].type) { // on事件
      if (dur) evtPush(unique(notes0)); // 不为零，则前一音符事件结束
      evtStart(i); // 加入事件
    } else if (flag > 1) flag--; // off事件
    else if (dur && notes0.length) evtPush(unique(notes0)); // flag为零，前一音符事件结束
  }
  dur = maxp - trkCur[trkCur.length - 1].p;
  if (dur > 0) evtPush(unique(notes0));
