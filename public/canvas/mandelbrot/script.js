'use strict';
self._i = ['Mandelbrot Set', [1, 0, 2], 1614954857, 1615280661];
...
canvas.addEventListener('mousedown', e => {
  ...
  init(kx, ky, dzoom += e.button ? -0.1 : 0.1, rowitr * 2 ** dzoom);
});
...
canvas.addEventListener('touchend', evt => {
  ...
  for (const i of evt.changedTouches) {
    ...
    init(kx, ky, dzoom += tm > 200 ? -0.1 : 0.1, rowitr * 2 ** dzoom);
  }
});
function resize() {
  canvas.width = size;
